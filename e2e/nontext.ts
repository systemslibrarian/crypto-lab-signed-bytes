 ...
 *
 * The first check earns its place in this repo immediately, and it replaces a
 * check that could not have found anything. `style.css` defines
 * `--control-border` as the boundary token and applied it to exactly two rules:
 * `#app textarea` and `#app input[type='text']`. Every `#app button` drew its
 * edge from `--border`, the SURFACE divider used for `#app section`, `#app code`
 * and every table rule, over a `--panel-soft` fill within 1.16:1 of the panel
 * behind it. Measured against the surface outside the control: 1.39:1 in dark,
 * 1.58:1 in light.
 *
 * The spec that was supposed to catch this, `e2e/border-contrast.spec.ts`, is
 * deleted, and it failed twice over. It queried
 * `#app textarea, #app input[type="text"]` — precisely and only the two
 * selectors `--control-border` was already applied to, so it asserted 3:1 over
 * the rules that already kept it. And it compared each element's
 * `borderTopColor` against its OWN `backgroundColor`, never against the surface
 * outside the control, so it did not ask the question 1.4.11 asks: whether the
 * control can be told apart from what surrounds it.
 *
 * The second check is live here too, unusually — see the note on generated
 * content in `contrast.ts`. Three `::after` rules draw the `→` connectors
 * between the nodes of both pipeline diagrams.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks eight mounted exhibits at once, and the expensive
     * part is that `hexDiffView()` renders ONE `<span class="b">` PER BYTE, twice
     * per stage. Without the caches the pass re-reads the same computed styles
