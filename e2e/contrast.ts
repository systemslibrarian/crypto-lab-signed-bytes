 ...
 *    an unknown backdrop, or a `color-mix()`. This page is built almost entirely
 *    out of `color-mix(in oklab, …)`, and every one of them is a surface that
 *    carries meaning rather than decoration: all three verdict chips
 *    (`.v-ok`, `.v-failclosed`, `.v-alarm` — the lights the whole lab is about),
 *    the JCS bar that gates every stage, `button.primary`, `.pipe-node.inside`
 *    and the `.current-row` highlight in Stage 5's tolerance matrix, `.cp-diff`
 *    on every differing code point, and `.hexbox .b.d` — the mark on every
 *    changed byte, which is the lab's central piece of evidence. The hero aside
 *    is another, and the shared top bar's `--cl-ink` is a
 *    `color-mix(in srgb, …)`. axe resolves none of them; it files them under
 *    `incomplete`. A violations-only gate therefore measured the contrast of
 *    almost nothing this lab renders.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This page has exactly one
 *    such rule, `.cl-hero-sub { opacity: 0.85 }`, on the subtitle directly
 *    beneath the H1. Through the composite it measures 11.13:1 dark and 9.74:1
 *    light, so it passes — but it passes as a measurement rather than as a
 *    reading of the declared value.
 *
 ...
 *    box is `.hexbox` — `overflow: auto` under a `max-height: 14rem` cap, and
 *    every hex view on the page lands in one. At 380px they scroll on both axes
 *    (the hex lines are `white-space: pre`, sixteen bytes wide), so a large part
 *    of every byte string is genuinely outside the frame. Content scrolled past
 *    that is not dimmed or partly drawn — it is absent, and asking what colour it
 *    sits on has no answer. Its rect is still outside every ancestor's box, so
 *    the ancestor walk would find nothing behind it and fall through to white,
 *    inventing a failure. Skip it, and rely on the content that IS in view, which
 *    is measured for real. The Stage 5 tolerance matrix is a `.hexbox` too, with
 *    an inline `max-height:none`, so it clips horizontally only.
 *
 ...
 *    draws both of its figures — the receive pipeline and the producer/gateway/
 *    verifier diagram — as CSS grids of `<div>`s rather than in SVG, so the
 *    sibling walk finds nothing to do. Its only SVG is the shared top bar's two
 *    `aria-hidden` marks. The branch is kept because the alternative is a helper
 *    that silently mismeasures the first labelled diagram anyone adds: an
 ...
 * not one — nor is it one to axe's `color-contrast` rule. This page has THREE
 * such rules and all of them draw a real glyph: `.pipe-node::after` and
 * `.mech-node::after` paint the `→` connectors between the nodes of both
 * diagrams (`↓` instead, below 640px), in `--text-dim`. They are the only thing
 * on screen saying the diagrams are pipelines rather than three loose boxes.
 *
 * `nontext.ts` measures generated content directly and runs at every driven
 * state, so unlike most labs in this fleet these are not hand-measured but
 * gated. The hand figures, for the record: `--text-dim` on the `--panel-soft`
 * fill the arrows overlap is 6.99:1 dark and 6.61:1 light, well clear of the
 * 4.5:1 a 1rem glyph is held to. They are absolutely positioned, so
 * `nontext.ts` reports them as `POSITIONED` — it measures them against the
 * host's backdrop, which is right here because the arrow sits over the
 * `.pipe-wrap` / `.mech-diagram` surface either way.
 */
 ...
 * uses. The one genuinely short-lived styling here is the `.uf-animate` class
 * on the HMAC state-update diagram, which drives a 0.5s fade and a 0.7s pulse —
 * and under the reduced motion this gate asserts, the stylesheet cancels both
 * (`animation: none`), so the class is present but paints nothing transient and
 * a scan cannot catch a half-faded box.
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks eight mounted exhibits at once, and the
     * expensive part is that `hexDiffView()` renders ONE `<span class="b">` PER
     * BYTE: every stage on the page shows two byte strings that way, so a single
     * scan walks well over a thousand sibling spans all re-walking the same
     ...
     * that is every `.hexbox` (`overflow: auto` under a 14rem cap), which is
     * where every byte string on the page is rendered.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details>` expert notes, one per stage, all of which ship shut. The gate
      ...
      // replaces did to all of them at once, before its single scan.
      ...
      // header's `.cl-skip-link` — the page's only one, since this lab adds no
      // skip link of its own — parks at `top:-3rem` and slides in only on focus.
      // Measuring the parked copy invents a failure for text that is not on
      // screen; the focused rendering is a real state and the gate scans it
      // explicitly instead.
      ...
      //
      // This lab does NOT use the idiom — `style.css` defines no `.sr-only`
      // rule and nothing in `src/ui` applies one, so the guard is inert here
      // today. That is recorded rather than left to be rediscovered: the guard
     ...
     *
     * This lab hides THREE things, and each was measured by hand rather than
     * assumed, because that is the shared blind spot where both oracles stop
     * looking:
     *
     *  - `.mech-diagram`, Exhibit 1's producer/gateway/verifier illustration.
     *    Its source comment says so explicitly: the diagram illustrates steps
     *    that carry the content themselves, and every one of those steps is
     *    exposed prose immediately below it. Its ink: `.mech-node` is `--text`
     *    on `--panel` (14.02:1 dark), and `.mech-packet` is `--accent-text` on
     *    `--panel-soft` (6.90:1 dark, 6.34:1 light).
     *  - `.pipe-badge`, the "✍ signed here" marker on Stage 5's selected tap.
     *    The same fact is in the slider's `aria-valuetext`, which the gate
     *    asserts changes with the boundary. Ink: `--accent-text` on
     *    `--panel` composited with an accent tint, clearing 6:1 in both themes.
     *  - `.cp-glyph`, the rendered character in each code-point cell. It is
     *    hidden deliberately and correctly: a lone U+0301 combining accent has
     *    no useful spoken form, and the cell's `U+XXXX` label — which is NOT
     *    hidden and IS measured — is the accessible content.
     *
     * Nothing on this page hides a VALUE. No byte string, no digest, no
     * signature, no verdict and no matrix cell is inside an `aria-hidden`
     * subtree — which was checked rather than assumed.
     */
     ...
     *
     * This lab has no SVG of its own — both figures are CSS grids of `<div>`s,
     * and every node is built through `dom.ts`'s `h()` helper. The guard stays
     * because it costs nothing and because the first SVG figure added would
     * otherwise be measured wrongly: text sitting directly in a `<g>` or an
     * `<svg>` is in the DOM but paints nothing, and a walk that measured it would
     ...
     *
     * It IS a no-op on this page today, and it is worth saying exactly why:
     * `style.css` sets `html { background-color: #141210 }` with an explicit
     * `html[data-theme='light']` override, so the first branch below returns an
     * opaque colour and the `<body>` fallback is never reached. That is one
     * deleted declaration away from breaking, on a page eight exhibits and many
     * screens deep whose dark surface is `#141210`: fall through to white
     * instead and every element below the first screen would suddenly be
